...
- The ground follows the time of day. The sky and hills went to dusk while the field stayed in full daylight, so the horizon was where the time of day changed; the field now takes the nearest hill band's colour in every phase, as the day palette already did.
- Dusk and night lay a wash over the whole scene. The scenery can be repainted but the sprites cannot — the pig and the flowers are drawn under one fixed daylight — so darkening only the ground left them glowing on top of it like cut-outs.
- A dev-only sandbox at `/dev/pig` (every clip, play/pause, frame stepping, speed) and `/dev/garden` (flower species, time of day, note count, growth stage, and whether a letter is waiting). The letter control is expressed as an age floor on the read notes rather than a flag, because that is the actual shape of the state — so the sandbox can only show gardens the product can really produce. Both live under a `/dev` layout that calls `notFound()` in production, so a new tool is dev-only by default rather than by remembering to gate it.
